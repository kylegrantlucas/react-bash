...
describe('fish commands', () => {
    ...
    describe('help', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.help.exec);
        });
        ...
        it('should handle no args', () => {
            ...
            const { history } = fish.commands.help.exec(state, {});
    ...
    describe('clear', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.clear.exec);
        });

        it('should clear out the history', () => {
            const state = stateFactory({ history: [1, 2] });
            const { history } = fish.commands.clear.exec(state, {});
    ...
    describe('ls', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.ls.exec);
        });
        ...
        it('should handle no args', () => {
            ...
            const expected = Object.keys(state.structure)
                .filter(name => name[0] !== '.')
                .join(' ');
            const { history } = fish.commands.ls.exec(state, { flags: {}, args: {} });
        ...
        it('should handle --all arg', () => {
            ...
            const { history } = fish.commands.ls.exec(state, command);
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
    ...
    describe('cat', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.cat.exec);
        });

        it('should display file contents', () => {
            const state = stateFactory();
            const expected = state.structure.file1.content;
            const { history } = fish.commands.cat.exec(state, { args: { 0: 'file1' } });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
        ...
        it('should display file contents from path', () => {
            ...
            const { history } = fish.commands.cat.exec(state, { args: { 0: 'dir1/dir1File' } });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
        ...
        it('should not work for directories', () => {
            ...
            const { history } = fish.commands.cat.exec(state, { args: { 0: 'dir1' } });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
        ...
        it('should not work for invalid paths', () => {
            const state = stateFactory();
            const expected = Errors.NO_SUCH_FILE.replace('$1', 'doesNotExist');
            const { history } = fish.commands.cat.exec(state, { args: { 0: 'doesNotExist' } });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
        ...
        it('should not work for nested invalid paths', () => {
            ...
            const { history } = fish.commands.cat.exec(state, { args: { 0: 'dir1/doesNotExist' } });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
        ...
        it('should not work for directory paths', () => {
            ...
            const { history } = fish.commands.cat.exec(state, { args: { 0: 'dir1/childDir' } });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
    ...
    describe('mkdir', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.mkdir.exec);
        });

        it('should create a new directory', () => {
            const state = stateFactory();
            chai.assert.isUndefined(state.structure.testDir);
            const { structure } = fish.commands.mkdir.exec(state, { args: { 0: 'testDir' } });
        ...
        it('should create a new directory at path', () => {
            ...
            const { structure } = fish.commands.mkdir.exec(state, { args: { 0: 'dir1/testDir' } });
        ...
        it('should not create a directory if it already exists', () => {
            ...
            const { history } = fish.commands.mkdir.exec(state, { args: { 0: 'dir1' } });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
        ...
        it('should not create a directory from path if it already exists', () => {
            ...
            chai.assert.isDefined(state.structure.dir1);
            const { history } = fish.commands.mkdir.exec(state, { args: { 0: 'dir1/childDir' } });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, expected);
        });
    ...
    describe('cd', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.cd.exec);
        });

        it('should work with no args', () => {
            const state = stateFactory();
            state.cwd = 'dir1';
            const { cwd } = fish.commands.cd.exec(state, { args: {} });
            chai.assert.strictEqual(cwd, '');
        });

        it('should work with path', () => {
            const state = stateFactory();
            const { cwd } = fish.commands.cd.exec(state, { args: { 0: 'dir1/childDir' } });
        ...
        it('should work with ..', () => {
            ...
            const { cwd } = fish.commands.cd.exec(state, { args: { 0: '..' } });
        ...
        it('should work with back and forth', () => {
            const state = stateFactory();
            state.cwd = 'dir1/childDir';
            const { cwd } = fish.commands.cd.exec(state, { args: { 0: '../childDir' } });
            chai.assert.strictEqual(cwd, 'dir1/childDir');
        });
    ...
    describe('cd', () => {
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.pwd.exec);
        });
        ...
        it('should print out cwd', () => {
            ...
            state.cwd = 'dir1/childDir';
            const { history } = fish.commands.pwd.exec(state);
    ...
    describe('echo', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.echo.exec);
        });

        it('should print out empty arguments', () => {
            const { history } = fish.commands.echo.exec(state, { input: 'echo' });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, '');
        });

        it('should print out arguments', () => {
            const { history } = fish.commands.echo.exec(state, { input: 'echo foo bar' });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, 'foo bar');
        });

        it('should print out static environment variables', () => {
            const { history } = fish.commands.echo.exec(state, { input: 'echo $HOME' });
            chai.assert.strictEqual(history.length, 1);
            chai.assert.strictEqual(history[0].value, '/');
        });

        it('should print out dynamic environment variables', () => {
            const { history } = fish.commands.echo.exec(state, { input: 'echo $PWD' });
            chai.assert.strictEqual(history.length, 1);
    ...
    describe('printenv', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.printenv.exec);
        });

        it('should print out the environment variables', () => {
            const { history } = fish.commands.printenv.exec(state, {});
    ...
    describe('whoami', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.whoami.exec);
        });

        it('should print out the environment variables', () => {
            const { history } = fish.commands.whoami.exec(state, {});
            chai.assert.strictEqual(history.length, 1);
    ...
    describe('rm', () => {
        ...
        it('should exist', () => {
            chai.assert.isFunction(fish.commands.rm.exec);
        });

        it('should delete files with the default command', () => {
            const state = stateFactory();
            chai.assert.isDefined(state.structure.file1);
            const { structure } = fish.commands.rm.exec(state, { args: { 0: 'file1' }, flags: { } });
        ...
        it('should not delete a folder without the appropriate flags', () => {
            const state = stateFactory();
            chai.assert.isDefined(state.structure.dir1);
            const { structure } = fish.commands.rm.exec(state, { args: { 0: 'dir1' }, flags: { } });
        ...
        it('should delete a folder with -r', () => {
            const state = stateFactory();
            chai.assert.isDefined(state.structure.dir1);
            const { structure } = fish.commands.rm.exec(state, { args: { 0: 'dir1' }, flags: { r: true } });
